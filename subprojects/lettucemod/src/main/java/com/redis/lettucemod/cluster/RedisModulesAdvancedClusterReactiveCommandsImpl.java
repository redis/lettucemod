...
public class RedisModulesAdvancedClusterReactiveCommandsImpl<K, V> extends
	...
	@Override
	public Mono<String> tDigestAdd(K key, double... value) { return delegate.tDigestAdd(key,value);	}

	@Override
	public Flux<Double> tDigestByRank(K key, long... ranks) { return delegate.tDigestByRank(key, ranks); }

	@Override
	public Flux<Double> tDigestByRevRank(K key, long... revRanks) { return delegate.tDigestByRevRank(key, revRanks); }

	@Override
	public Flux<Double> tDigestCdf(K key, double... values) { return delegate.tDigestCdf(key, values);	}

	@Override
	public Mono<String> tDigestCreate(K key) { return delegate.tDigestCreate(key); }

	@Override
	public Mono<String> tDigestCreate(K key, long compression) { return delegate.tDigestCreate(key, compression); }

	@Override
	public Mono<TDigestInfo> tDigestInfo(K key) { return delegate.tDigestInfo(key);	}

	@Override
	public Mono<Double> tDigestMax(K key) { return delegate.tDigestMax(key);	}

	@Override
	public Mono<String> tDigestMerge(K destinationKey, K... sourceKeys) { return delegate.tDigestMerge(destinationKey, sourceKeys);	}

	@Override
	public Mono<String> tDigestMerge(K destinationKey, TDigestMergeOptions options, K... sourceKeys) { return delegate.tDigestMerge(destinationKey, options, sourceKeys); }

	@Override
	public Mono<Double> tDigestMin(K key) { return delegate.tDigestMin(key);	}

	@Override
	public Flux<Double> tDigestQuantile(K key, double... quantiles) { return delegate.tDigestQuantile(key, quantiles);	}

	@Override
	public Flux<Long> tDigestRank(K key, double... values) { return delegate.tDigestRank(key, values);	}

	@Override
	public Mono<String> tDigestReset(K key) { return delegate.tDigestReset(key);	}

	@Override
	public Flux<Long> tDigestRevRank(K key, double... values) { return delegate.tDigestRevRank(key, values);	}

	@Override
	public Mono<Double> tDigestTrimmedMean(K key, double lowCutQuantile, double highCutQuantile) { return delegate.tDigestTrimmedMean(key,lowCutQuantile,highCutQuantile); }
	...
	@Override
	public Mono<Boolean> bfAdd(K key, V item) { return delegate.bfAdd(key, item); }

	@Override
	public Mono<Long> bfCard(K key) { return delegate.bfCard(key); }

	@Override
	public Mono<Boolean> bfExists(K key, V item) { return delegate.bfExists(key,item); }

	@Override
	public Mono<BfInfo> bfInfo(K key) { return delegate.bfInfo(key); }

	@Override
	public Mono<Long> bfInfo(K key, BfInfoType infoType) { return delegate.bfInfo(key, infoType); }

	@Override
	public Flux<Boolean> bfInsert(K key, V... items) { return delegate.bfInsert(key, items); }

	@Override
	public Flux<Boolean> bfInsert(K key, BfInsertOptions options, V... items) { return delegate.bfInsert(key, options, items);}

	@Override
	public Flux<Boolean> bfMAdd(K key, V... items) { return delegate.bfMAdd(key, items); }

	@Override
	public Flux<Boolean> bfMExists(K key, V... items) { return delegate.bfMExists(key, items); }

	@Override
	public Mono<String> bfReserve(K key, BfConfig config) { return delegate.bfReserve(key, config);	}

	@Override
	public Mono<Boolean> cfAdd(K key, V item) { return delegate.cfAdd(key,item); }

	@Override
	public Mono<Boolean> cfAddNx(K key, V item) { return delegate.cfAddNx(key,item);	}

	@Override
	public Mono<Long> cfCount(K key, V item) {return delegate.cfCount(key,item);	}

	@Override
	public Mono<Boolean> cfDel(K key, V item) { return delegate.cfDel(key,item); }

	@Override
	public Mono<Boolean> cfExists(K key, V item) { return delegate.cfExists(key,item); }

	@Override
	public Mono<CfInfo> cfInfo(K key) { return delegate.cfInfo(key); }

	@Override
	public Flux<Long> cfInsert(K key, V... items) { return delegate.cfInsert(key,items);	}

	@Override
	public Flux<Long> cfInsert(K key, CfInsertOptions options, V... items) { return delegate.cfInsert(key, options, items);	}

	@Override
	public Flux<Long> cfInsertNx(K key, V... items) { return delegate.cfInsertNx(key,items);	}

	@Override
	public Flux<Long> cfInsertNx(K key, CfInsertOptions options, V... items) { return delegate.cfInsertNx(key, options,items); }

	@Override
	public Flux<Boolean> cfMExists(K key, V... items) { return delegate.cfMExists(key,items); }

	@Override
	public Mono<String> cfReserve(K key, Long capacity) { return delegate.cfReserve(key, capacity);	}

	@Override
	public Mono<String> cfReserve(K key, CfReserveOptions options) { return delegate.cfReserve(key,options); }

	@Override
	public Mono<Long> cmsIncrBy(K key, V item, long increment) { return delegate.cmsIncrBy(key,item,increment);	}

	@Override
	public Flux<Long> cmsIncrBy(K key, Map<V, Long> increments) { return delegate.cmsIncrBy(key,increments); }

	@Override
	public Mono<String> cmsInitByProb(K key, double error, double probability) { return delegate.cmsInitByProb(key,error,probability); }

	@Override
	public Mono<String> cmsInitByDim(K key, long width, long depth) { return delegate.cmsInitByDim(key, width, depth); }

	@Override
	public Flux<Long> cmsQuery(K key, V... items) { return delegate.cmsQuery(key,items); }

	@Override
	public Mono<String> cmsMerge(K destKey, K... keys) { return delegate.cmsMerge(destKey,keys);	}

	@Override
	public Mono<String> cmsMerge(K destKey, Map<K, Long> keyWeightMap) { return delegate.cmsMerge(destKey,keyWeightMap);	}

	@Override
	public Mono<CmsInfo> cmsInfo(K key) { return delegate.cmsInfo(key); }
}
